...
%\newglossary[flog]{flabel}{fout}{fin}{Flowers}
\newglossary[flog]{cat}{fout}{fin}{Glossary of Cats and Dogs}
\newglossary[plog]{pos}{pout}{pin}{Parts of Speech}
\makeglossaries

\usepackage{xcolor,luacolor,lua-ul}
\usepackage{fontspec}
\newfontfamily\glrubytext{LinuxLibertineDisplayG}
...
\usepackage{xurl}
\newcommand\surl[1]{{\footnotesize\noindent Source Reference}{{\color{brown}\\\url{#1}}}}
%CJK:
...
\newfontface\fug{NotoSansUgaritic-Regular}[Path=C:/Users/epam2/Downloads/fonts/noto/,Extension=.ttf]
\newfontface\fsym{SegoeUISymbol}
...
\newcommand\uplbrace{\textup{\{}}
\newcommand\uprbrace{\textup{\}}}
...
%========================
		...
		\g__gloss_keepdotpos_bool
		{
			...
			{
%    .text  >  .{\mfsposformat text} : dot true, glossary true
		\regex_replace_all:nnN
				{ (\.)(\w+) }
				{ \1 \cB\{ \c{mfsposformat} \c{gls}\cB\{ \2 \cE\} \cE\}
					\c{makenewge}\cB\{ \2 \cE\}
%					\c{gleat}\cB\{\c{gls}\cB\{ \2 \cE\}\cE\}
%%					\c{gls}\cB\{ \2 \cE\}
				} % glossary for pos
				\l_tmpc_tl
			}
...
%    .text-text2;  >  .{\mfsposformat \text2{text}} : dot true, glossary false
		\regex_replace_all:nnN
				{ (\.)(\w+)(\-)(\w+)(;) }
...
%    .text  >  .{\mfsposformat text}
		...
		{
%    .text  >  {\mfsposformat text} : dot false
		\regex_replace_all:nnN
				{ (\.)(\w+)(\-)(\w+)(;) }
...
%%%		:speaker+word xxx	 > word:_	(speaker label)
%%		\regex_replace_all:nnN
%%				{ (\.\.)(\w+)(\,\,)(\w+) }
%%%				\\ \ \kern-2em{\textbf{99}}:
%%%				\ \kern-2em{\textbf{99}}:
%%				{
%%%				 \c{\\}\space\c{kern}-2em\c{textbf\cB\{\c{\2}\cE\}\space\4 
%%					XXX
%%				 }
%%				\l_tmpc_tl
...
%
%\box_if_exist:cF
%	{ l_gloss_store \l__wordloop_int _box }
%	{
%	  \box_new:c 
%	  		{ l_gloss_store \l__wordloop_int _box }
%	}
...
%=================
\NewDocumentCommand{ \mfssortaseq } { o m } { 
% 1=namespace
% 2=seq name
...
%--------------------
\NewDocumentCommand { \mfsaddaprop } { o m m m } { % 
% 1=NS,
% 2=prop name
% 3 = key
% 4 = value
...
\tl_new:N \g_fc_namespacea_tl
\tl_new:N \g_fc_namespaceb_tl
%\int_new:N \l_gloss_loopstart_int
\int_new:N \l_gloss_loopfinish_int
\int_new:N \l_gloss_loopcount_int
\seq_new:N \l_gloss_defnrecord_seq

%--------------------
\NewDocumentCommand { \mfsseqtoprop } { o m o m } { %
% 1 = NS seq,
% 2 = sqq name
% 3 = NS prop
% 4 = prop name
				...
				\IfNoValueTF { #1 } 
						{ \tl_clear:N \g_fc_namespacea_tl } 
						{ \tl_gset:Nn \g_fc_namespacea_tl { #1 } }

				\IfNoValueTF { #3 } 
						{ \tl_clear:N \g_fc_namespaceb_tl } 
						{ \tl_gset:Nn \g_fc_namespaceb_tl { #3 } }

		\cs_if_free:cT
				{ g_fc_rwe \g_fc_namespaceb_tl #4 _prop }
			{ \prop_new:c
				{ g_fc_rwe \g_fc_namespaceb_tl #4 _prop }
			}
	...
	\int_set:Nn 
			\l_gloss_loopcount_int 
			{ 1 }
	\int_set:Nn 
			\l_gloss_loopfinish_int 
			{ \seq_count:c { g_fc_rwe \g_fc_namespacea_tl #2 _seq }
			}

%			\int_set:Nn 
%				\l_gloss_loopcount_int { 1 }
%	\int_show:N
%			\l_gloss_loopfinish_int
...
%	\int_compare:nNnT
	\int_step_inline:nnnn
		...
		{ \l_gloss_loopfinish_int }
		{
			\seq_set_split:Nnx % get the next K;V record
				 \l_gloss_defnrecord_seq 
				{ ; } % separator
				{ \seq_item:cn 
					{ g_fc_rwe \g_fc_namespacea_tl #2 _seq } 
					{ ##1 }
				 }
...
%%\seq_show:N 					\l_gloss_defnrecord_seq 

			\prop_gput:cxx % store as KV
				{ g_fc_rwe \g_fc_namespaceb_tl #4 _prop }
				{ \seq_item:Nn 
					\l_gloss_defnrecord_seq 
					{ 1 }
...
\tl_new:N \l_gloss_name_tl
\tl_new:N \l_gloss_desc_tl

\NewDocumentCommand { \makenewge } { m } {%


%	\prop_show:c
%			{ g_fc_rweleipzigglossing_prop } 
%	\tl_set:Nv
%		\l_gloss_name_tl
%		{ #1 }
	\tl_set:Nn
		\l_gloss_desc_tl
		{
			\mfsgetapropkv[leipzig]{glossing}{ #1 }
		}
%	\tl_show:N
%		\l_gloss_desc_tl
	\newglossaryentry
		...
		type=pos,
		name={ #1 },
		description={ \mfsgetapropkv[leipzig]{glossing}{ #1 } },%\tl_use:N \l_gloss_desc_tl },
		}
}
...
\NewDocumentCommand { \glspeaker } { m +m } { 
	% 1==speaker
	% 2==utterance
	\par\medskip\textbf{#1} ~ ~
	\glinlines { #2 }
...
\NewDocumentCommand { \gltwoex } { O{0.35} O{0.50} +m +m } { 
	% 1==width % ex1
	% 2==width % ex2
	% 3==ex1
	% 4==ex2
...
%%%
\cs_set_eq:NN \dofileinput \file_input:n

%%%
\NewDocumentCommand { \glmarker } { O{gloss} O{circnum} m } {
				\IfNoValueTF { #1 } 
						{ \tl_clear:N \g_fc_namespace_tl } 
						{ \tl_gset:Nn \g_fc_namespace_tl { #1 } }
...
\NewDocumentCommand { \glrange } { m } { %{ O{gloss} O{ex2a} m m } {
		\box_use:c
%			{ l_gloss_store \l__wordloop_int _box }
			{ l_gloss_store #1 _box }
...
%   \seq_set_eq:Nc
%   			\l_tmpa_seq 
%			{ g_gloss \g_fc_namespace_tl #2 _seq } 
%
%	Seq ~ #1/#2 ~ has ~
...
%\newcommand\dauq{\hangindent=4em\hangafter=-1}
\newcommand\gleat[1]{}
...
\dofileinput{in-glossing.tex}
\mfssortaseq[leipzig]{glossing}
\mfsseqtoprop[leipzig]{glossing}[leipzig]{glossing}
%\mfsuseaprop[leipzig]{glossing}

%
% circled numbers

\mfsloadaseq[gloss]{circnum}{%
①,②,③,④,⑤,⑥,⑦,⑧,⑨,⑩
,⑪,⑫,⑬,⑭,⑮,⑯,⑰,⑱,⑲,⑳
}
